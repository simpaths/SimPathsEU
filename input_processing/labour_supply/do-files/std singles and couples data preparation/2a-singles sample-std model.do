...
*			Labour supply estimation-get sample for single and generate the vbls needed for ls (std singles)
*************************************************************************


global file_log="${log}/singles_sample-std model"
...
//update 1/2/2021: at the end of the do file, separate the sample into two subsamples: singles_indep and singles_dep
...
label var lhw "weekly hours worked"

//assert bunct_s==0 //N/A for HU: making sure unemployment benefit (JSA): bunct_s=0 for every alternatives for everyone
...
assert _merge==3
drop _merge
duplicates report temp_idorigperson2 if temp_singles==1 //count how many unique individuals there are
di r(unique_value)
...
lab var hrs_40plus_dgn "1(weekly working hours>=40)\times$1(male)"
...
//subsample: singles_indep
preserve
keep if temp_singles_indep==1
save singles_indep_wage`i',replace
restore
//subsample: singles_indep
preserve
keep if temp_singles_dep==1
save singles_dep_wage`i',replace
restore

save singles_wage`i',replace  //still save this, as will be used in EM sample (1.1wage)
